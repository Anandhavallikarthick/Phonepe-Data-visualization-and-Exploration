...
if selected == "Home":
    ...
    with col1:
        ...
        st.markdown("### :violet[Technologies used :] Github Cloning, Python, Pandas,  mysql-connector-python, Streamlit, and Plotly.")
        st.markdown("### :violet[Overview :] In this streamlit web app you can visualize the phonepe pulse data and gain lot of insights on transactions, number of users, top 10 state, district, pincode and which brand has most number of users and so on. Bar charts,sunburst charts, Pie charts, Donut charts and Geo map visualization are used to get some insights.")
...
# Rendering Markdown content with Streamlit
        ...
        selected_year_1 = st.slider("Select Year 1", min_value=2018, max_value=2023, key="year_1")
        ...
        with col2:
            ...
            mycursor.execute(f"select districts , sum(Transaction_Count) as Total_Count, sum(Transaction_Amount) as Total from map_transaction2 where years = {selected_year_1} and quarter = {selected_quarter} group by districts order by Total desc limit 10")
...
# Top Charts - USERS          
    if Type == "Users":
        ...
        mycursor.execute(f"select States, Districts,years,quarter, sum(Transaction_count) as Total_Transactions, sum(Transaction_amount) as Total_amount from map_transaction2 group by States, Districts order by Total_Transactions desc limit 10")
...
# EXPLORE DATA - TRANSACTIONS
    if Type == "Transactions":
        ...
        with col1:
            ...
            mycursor.execute(f"select states, sum(Transaction_count) as Total_Transactions, sum(Transaction_amount) as Total_amount from map_transaction2 where years = {Year} and quarter = {Quarter} group by states order by states")
        ...
        with col2:
            ...
            #This is a pandas method (astype()) used to cast the values of a column to a specified data type.
...
# BAR CHART TRANSACTIONS - DISTRICT WISE DATA            
        ...
        mycursor.execute(f"select States, Districts,years,quarter, sum(Transaction_count) as Total_Transactions, sum(Transaction_amount) as Total_amount from map_transaction2 where years = {Year} and quarter = {Quarter} and States = '{selected_state}' group by States, Districts,years,quarter order by states,districts")
        ...
        mycursor.execute(f"select States, Districts,years,quarter, sum(Transaction_count) as Total_Transactions, sum(Transaction_amount) as Total_amount from map_transaction2 where years = {Year} and quarter = {Quarter} and States = '{selected_state}' group by States, Districts,years,quarter order by Total_Transactions DESC limit 10")
        ...
        fig2 = px.pie(df1, values='Total_Transactions', names='District',title=selected_state, hole=0.4
                    )
...
#sunburst chart
        ...
        mycursor.execute(f"select States, Districts,years,quarter, sum(Transaction_count) as Total_Transactions, sum(Transaction_amount) as Total_amount from map_transaction2 where years = {Year} and quarter = {Quarter} and States = '{selected_state}' group by States, Districts,years,quarter order by states,districts limit 10")
